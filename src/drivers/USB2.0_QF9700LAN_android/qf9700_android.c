...
static int qf_read(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	netdev_dbg(dev, "qf_read() reg=0x%02x length=%d", reg, length);
...
static int qf_write(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	netdev_dbg(dev, "qf_write() reg=0x%02x, length=%d", reg, length);
...
{
	netdev_dbg(dev, "qf_write_reg() reg=0x%02x, value=0x%02x", reg, value);
...
{
	netdev_dbg(dev, "qf_write_async() reg=0x%02x length=%d", reg, length);
...
{
	netdev_dbg(dev, "qf_write_reg_async() reg=0x%02x value=0x%02x", reg, value);
...
static int qf_share_read_word(struct usbnet *dev, int phy, u8 reg, __le16 *value)
{
	...
	netdev_dbg(dev, "read shared %d 0x%02x returned 0x%04x, %d",
	       phy, reg, *value, ret);
...
static int qf9700_android_mdio_read(struct net_device *netdev, int phy_id, int loc)
{
	...
	if (phy_id) {
		netdev_dbg(dev, "Only internal phy supported");
		return 0;
	}
...
static void qf9700_android_mdio_write(struct net_device *netdev, int phy_id, int loc, int val)
{
	...
	netdev_dbg(dev,"qf9700_android_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
	       phy_id, loc, val);
...
static void qf9700_android_status(struct usbnet *dev, struct urb *urb)
{
	...
	*/
	...
	if (netif_carrier_ok(dev->net) != link) {
		...
		netdev_dbg(dev, "Link Status is: %d", link);
	}
}
...
static int qf9700_android_link_reset(struct usbnet *dev)
{
	...
	netdev_dbg(dev, "link_reset() speed: %d duplex: %d",
